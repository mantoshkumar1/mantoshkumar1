# Mantosh Kumar

Staff Software Engineer

Building platforms, automation systems, and developer tools that help engineering teams move faster.


## About

I am a Staff Software Engineer with 14+ years of experience designing and building backend systems, automation infrastructure, developer platforms, and engineering productivity solutions.

My career has spanned engineering organizations across India, Germany, and Canada:

* Nokia (Canada)
* Intel, Siemens, KI Labs (Germany)
* Cisco, Aricent (India)

I hold an M.Sc. in Computer Science from the Technical University of Munich and a Bachelor of Technology in Computer Science.


## Current Focus

* Platform Engineering
* Distributed Systems
* Backend Development
* Engineering Productivity
* Automation Infrastructure
* Observability
* AI-Assisted Engineering Tools



## Featured Projects

### PhotoSahi

Generate passport and visa photos that comply with official requirements.

* Demo: https://mantoshkumar1.github.io/photosahi/
* Source: https://github.com/mantoshkumar1/photosahi

### Zerodha GTT Calculator

Utility for calculating valid Zerodha GTT trigger prices.

* Demo: https://gtt-calculator.streamlit.app/
* Source: https://github.com/mantoshkumar1/gtt-price-calculator

### Workflow Automation Toolkit

A collection of Python automation tools for repetitive document and workflow tasks.

Features include:

* PDF compression

* Image to PDF conversion

* Image optimization

* PDF page snapshots

* Batch document processing

* Workflow automation

* Source: https://github.com/mantoshkumar1/automated-Life

### Distributed Systems Project

Currently building a distributed systems project focused on understanding scalability, reliability, event-driven architectures, and system design through implementation.



## Writing

Topics I write about:

* Distributed Systems
* Platform Engineering
* Software Architecture
* Backend Engineering
* Automation
* AI Engineering
* Bitcoin and Blockchain



## Highlights

* Staff Software Engineer
* 14+ Years of Industry Experience
* M.Sc. Computer Science, Technical University of Munich
* Intel Heroes of Tomorrow Award (Twice)
* GATE Computer Science – Top 1% Nationwide (India)



## Connect

* GitHub: github.com/mantoshkumar1
* LinkedIn: https://www.linkedin.com/in/mantoshk/
* Website: Coming Soon
